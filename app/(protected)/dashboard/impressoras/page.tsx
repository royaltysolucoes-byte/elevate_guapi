...
export default function ImpressorasPage() {
  ...
  useEffect(() => {
    fetchImpressoras(true); // Mostra loading apenas na mudança de página
  }, [page]);
  ...
  const fetchImpressoras = async (showLoading = false) => {
    try {
      if (showLoading) setLoading(true);
      console.log('Buscando impressoras...');
      const searchParam = searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : '';
      const response = await fetch(`/api/impressoras?page=${page}${searchParam}`);
      const data = await response.json();
      if (response.ok) {
      ...
      <form onSubmit={handleSearch} className="mb-6">
        <div className="flex gap-2">
          <input
            ...
            placeholder="Buscar por setor, número de série, IP ou categoria..."
